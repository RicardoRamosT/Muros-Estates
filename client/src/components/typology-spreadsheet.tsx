      ...
      >
        ...
        >
          <SelectTrigger className={`h-6 w-full text-xs border-0 bg-transparent px-0 !justify-center gap-0.5 [&_svg]:h-3 [&_svg]:w-3 [&_svg]:shrink-0 focus:ring-0 focus:ring-offset-0 ${textColorClass}`} data-testid={`boolean-${column.key}-${rowId}`}>
            <span className="shrink-0 text-left font-bold" style={{ width: '2.5ch' }}>{value === true ? "Sí" : value === false ? "No" : (canBeUnassigned ? "-" : "-")}</span>
          </SelectTrigger>
          <SelectContent>
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            <span className="truncate min-w-0 flex-1 font-bold">{displayValue || (column.allowUnassigned ? "-" : "")}</span>
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="__clear__" className={column.allowUnassigned ? "text-black font-bold italic" : "text-muted-foreground italic"}>
              {column.allowUnassigned ? <span className="text-black font-bold">-</span> : <span className="opacity-50">—</span>}
            </SelectItem>
