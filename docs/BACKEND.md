...
### Admin Seeding
On startup, if no admin exists: creates one with `ADMIN_INITIAL_PASSWORD` env var or a random password (logged to console).
...
| Method | Path | Auth | Description |
|--------|------|------|-------------|
| GET | `/api/developers` | No | List developers |
...
| Method | Path | Auth | Description |
|--------|------|------|-------------|
| GET | `/api/development-media` | Yes | Get media (filter by typologyId) |
...
interface IStorage {
  ...
  // Sessions
  createSession(userId), getSession(id), deleteSession(id), deleteExpiredSessions()
  ...
  // Documents, Media, SharedLinks, Catalogs, Permissions, Notifications...
}
